...
但是 Android 是纯客户端使用的，在 SDK 初始化的时候不允许传入 Master Key，我们只推荐在服务端使用 Master Key 发送请求。因此我们推荐有自定义管理后台需求的应用可以使用 [LeanEngine](https://leancloud.cn/docs/leanengine_overview.html) 开发和托管。 
